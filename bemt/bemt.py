...
def bemt_forward_flight_old(quadrotor, pitch, omega, alpha, v_inf, n_azi_elements, alt=0, tip_loss=True, mach_corr=False,
    ...
    for i_azi, azi_ang in enumerate(psi):
        ...
        while not all(converged) and i < max_i:
            ...
            if tip_loss:
                ...
                #F[np.isnan(F)] = 0.999999
            try:
                ff_inflow = inflow.new_ff(local_inflow, local_solidity, lambda_c, local_angle, alpha, mu, alpha0,
                                          Clalpha, r, F, local_mach, mach_corr=mach_corr)
                ff_inflow_perturb = inflow.new_ff(local_inflow+eps, local_solidity, lambda_c, local_angle, alpha, mu,
                                                  alpha0, Clalpha, r, F, local_mach, mach_corr=mach_corr)
                u_p = ff_inflow * v_tip
        ...
        if not all(converged):
            #print "ff inflow did not converge in BEMT"
